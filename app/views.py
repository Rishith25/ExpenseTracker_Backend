...
from .models import CustomUser, Account, FinancialTransaction
...
class AccountView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {"error": "Account with this account no already exists"},
...
class TransactionView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        account = Account.objects.filter(
            user=user_id, account_no=int(request.data["account_no"])
        ).first()
...
class AnalyticsDataView(generics.CreateAPIView):
    ...
    def get(self, request):
        ...
        grouped_expenses = expenses_by_month.values("year", "month").annotate(
            total_amount=Sum("amount"), num_transactions=Count("id")
        )
        ...
        return Response(
            {"expenses": grouped_expenses, "incomes": grouped_incomes},
            status=status.HTTP_200_OK,
        )
